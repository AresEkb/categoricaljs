...
function GraphCategoryView(model) {
  ...
  this.start = function () {
    ...
    model.dom().objects().forEach(function (name) {
      ...
      // Add object elements
      //var prev = '_l_'+name;
      //var t = name+'['+obj.nodes.representative()+']';
      //data.links.push({ source : prev, target : t, inner : true, value : 1, ordinal : 0 });
      ...
      obj.edges.forEach(function (edge) {
        var s = name+'['+obj.source.image(edge)+']';
        var t = name+'['+obj.target.image(edge)+']';
        //var elNode = { name : elName, label : el, group : groupId, group_data : objNode };
        //objNode.nodes.push(elNode);
        //data.nodes.push(elNode);
        data.links.push({ source : s, target : t, element : true, value : 1, ordinal : 0 });
        //prev = name+'['+el+']';
      });
    });
    ...
    model.dom().morphisms().forEach(function (name) {
      ...
      dst.dom().nodes.forEach(function (a) {
        var b = dst.mapNode(a);
        var s2 = s+'['+a+']';
        var t2 = t+'['+b+']';
        if (!adjMatrix[s2]) {
    ...
    model.dom().morphisms().forEach(function (name) {
      ...
      if (dst.dom().nodes.isEmpty() || dst.codom().nodes.isEmpty()) {
        var sl = '_l_'+s;
        var tl = '_l_'+t;
...
cl(data);
  ...
  function init() {
    ...
    var linkEnter = linkData.enter()
      .append('g')
        .attr('class', function (d) { return "link" + (d.element ? ' link-element' : ' link-morphism'); });
    linkEnter
      .append('svg:path')
    ...
    force
      ...
      .force("ltr", function force(alpha) {
        ...
        for (var i = 0; i < net.nodes.length; i++) {
          var d = net.nodes[i];
          if (d.group_data) {
            if (isUndefined(gr[d.group])) {
              gr[d.group] = { x: d.x, y: d.y };
            }
            else {
              gr[d.group] = { x: (gr[d.group].x + d.x) / 2, y: (gr[d.group].y + d.y) / 2 };
            }
          }
        }
        for (var i = 0; i < net.nodes.length; i++) {
          var d = net.nodes[i];
          if (d.group_data) {
            d.x += (gr[d.group].x - d.x) * alpha / 5;
            d.y += (gr[d.group].y - d.y) * alpha / 5;
          }
        }
